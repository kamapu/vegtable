...
#' 
setMethod("cover_trans", signature(x = "character",
...
setMethod("cover_trans", signature(x = "factor",
				conversion = "coverconvert"),
...
setMethod("cover_trans", signature(x = "numeric",
				conversion = "coverconvert"),
		function(x, conversion, ...) {
			x <- paste(x)
			cover_trans(x, conversion, ...)
...
setMethod("cover_trans", signature(x = "vegtable", conversion = "missing"),
				...
				if(i %in% colnames(x@samples)) {
					...
					x@samples[Selection2,to] <-
							cover_trans(x@samples[Selection2, i],
									x@coverconvert, from = i, rule = rule,
							zeroto = zeroto)
				}
            return(x)
        }
