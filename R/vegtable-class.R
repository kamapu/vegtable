...
setClass("vegtable",
    slots = c(
      ...
      if(nrow(object@samples) > 0 & any(!c("ReleveID","TaxonUsageID") %in%
        ...
        return(paste("Columns 'ReleveID' and 'TaxonUsageID' are",
      ...
      if(!all(object@samples$ReleveID %in% object@header$ReleveID))
        return(paste("Some releve IDs from slot 'samples' are missing",
                "in slot 'header'"))
        ...
        for(i in names(object@syntax)) {
          if(paste("syntax", i, sep = "_") %in% colnames(object@header)
            ...
            if(!all(noNA %in% object@syntax[[i]][[i]]))
              return(paste0("Not all values of 'syntax_", i,
